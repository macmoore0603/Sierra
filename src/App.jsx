...
function App() {
    ...
    // Local Sierra router telemetry: last on-device route decision and its
    // confidence / latency. Surfaced as a subtle bottom-corner pill so users
    // can see when an utterance is being handled on-device by the
    // FunctionGemma router vs. being forwarded to Gemini Live.
    const [lastLocalRoute, setLastLocalRoute] = useState(null);
    ...
    useEffect(() => {
        ...
        socket.on('sierra_route', (data) => {
            // data: {function, arguments, confidence, latency_ms, is_fallback}
            setLastLocalRoute({ ...data, at: Date.now() });
        });
        socket.on('sierra_route_error', (data) => {
            console.warn('[Sierra Router]', data?.msg);
        });
        ...
        return () => {
            ...
            socket.off('sierra_route');
            socket.off('sierra_route_error');
            socket.off('cad_data');
    ...
    return (
        <div className="h-screen w-screen bg-black text-cyan-100 font-mono overflow-hidden flex flex-col relative selection:bg-cyan-900 selection:text-white">
            ...
            <div className="flex-1 relative z-10 flex flex-col items-center justify-center">
                ...
                {/* On-device Sierra router telemetry pill. Visible only after
                    the local FunctionGemma router has produced at least one
                    route decision. Non-interactive, bottom-left corner. */}
                {lastLocalRoute && (
                    <div className="absolute bottom-3 left-3 z-30 flex items-center gap-2 rounded-full border border-cyan-500/30 bg-black/60 backdrop-blur px-3 py-1 text-[10px] uppercase tracking-widest text-cyan-200 font-mono pointer-events-none select-none">
                        <span className={`w-1.5 h-1.5 rounded-full ${lastLocalRoute.is_fallback ? 'bg-yellow-400' : 'bg-emerald-400'}`} />
                        <span className="opacity-70">router</span>
                        <span className="text-cyan-100">{lastLocalRoute.function}</span>
                        <span className="opacity-50">{Math.round(lastLocalRoute.confidence * 100)}%</span>
                        <span className="opacity-40">{Math.round(lastLocalRoute.latency_ms)}ms</span>
                    </div>
                )}
            </div>
        </div>
