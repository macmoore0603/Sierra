"""
Sierra Backend Server (FastAPI + Socket.IO)

This is the central backend for the Sierra Electron app.

GOD MODE / PERVASIVE FULL ACCESS:
- When God Mode is active (the default and intended experience), voice wake,
  gestures, face auth, and personal/system integrations should be auto-activated.
- Safety/confirmation logic exists but is intentionally relaxed for non-destructive
  high-privilege actions in God Mode.
- The frontend is expected to never present core capabilities as "off" or restricted.

See GOD_MODE.md for the complete philosophy and auto-force requirements.
"""
...
# Kick off the local Sierra router model load in the background so the very
# first user utterance doesn't pay the cold-start latency. Falls back
...
async def _emit_local_route(sid, text):
    """Run text through the on-device router and emit telemetry to the UI.

    This is intentionally a *non-blocking hint* -- we still forward the
    utterance to Gemini below. The local router is used so the UI can show
    instant intent feedback ("Sierra heard: timer / 5 minutes") and so
    future work can short-circuit confident routes without a Gemini
    round-trip.
    """
    ...
    if router is None or not text:
        return
    try:
        # Run the (potentially synchronous, CPU/GPU heavy) route in a thread
...
async def user_input(sid, data):
    ...
    # Fire-and-forget local routing for telemetry / fast-path hints.
    asyncio.create_task(_emit_local_route(sid, text))
