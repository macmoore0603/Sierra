...
@sio.event
async def route_query(sid, data):
    """Headless endpoint: route a single text query through the local model.

    Returns the structured route to the client without touching Gemini. Useful
    for scripted callers, smoke tests, and the future on-device fast path.
    """
    ...
    if router is None:
        await sio.emit(
            'sierra_route_error',
            {'msg': 'Local router unavailable; falling back to Gemini.'},
            room=sid,
        )
        return
    try:
...
async def discover_printers(sid):
    ...
    if not audio_loop or not audio_loop.printer_agent:
        ...
        if saved_printers:
            ...
            await sio.emit('status', {'msg': "Connect to Sierra to enable printer discovery"})
            return
